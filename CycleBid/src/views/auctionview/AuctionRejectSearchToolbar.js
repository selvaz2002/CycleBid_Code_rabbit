...
const AuctionRejectSearchToolbar = props => {
  ...
  const getFormattedDateRange = () => {
    ...
    const dateGap = `${years > 0 ? `${years} year${years > 1 ? 's' : ''}` : ''} ${months > 0 ? `${months} month${months > 1 ? 's' : ''}` : ''} ${days > 0 ? `${days} day${days > 1 ? 's' : ''}` : ''}`.trim()
    return dateGap;
  };
...
export default AuctionRejectSearchToolbar
