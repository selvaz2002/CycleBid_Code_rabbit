...
import { formatDate,getRemainingTime1,formatDay } from 'src/@core/utils/getRemainingTime'
...
const userDetails = ({ userId }) => {
  ...
  const handleBack = () => {
    router.push(`/users/userview/${localStorage.getItem('userId')}`)
  }
  ...
  const handleClose = (event, reason) => {
    ...
    if (successMessage === 'Deleted Successfully' || errorMessage === 'Data not Found') {
      router.push(`/users/userview/${localStorage.getItem(userId)}`)
    }
